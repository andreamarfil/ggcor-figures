...
# apply correlation method and generate a data frame with columns: var1, var2, p, r, range p (ifelse p), range r (ifelse r)
...
quickcor(varechem, type = "upper", cor.test = T, method = "spearman", show.diag = F) +
  ...
  add_link(df = mantel, 
           ...
           curvature = F) +
  ### cross-out non-significant cells (redundant)
